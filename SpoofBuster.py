# -*- coding: utf-8 -*-
"""
Created on Fri Nov 26 16:07:59 2021
...
class SpoofBuster():
  ...
  def fit_model(self, x, y=None, ep=10, pat=3, val=0.1,
    ...
    if val_data is None:
      if y is None:
        self._model.fit(x, validation_split=val, batch_size=100, epochs=ep, verbose=1, class_weight=class_weights,
                        callbacks=[callback])
      else:
        self._model.fit(x, y, batch_size=100, epochs=ep, validation_split=val, verbose=1, class_weight=class_weights,
                        callbacks=[callback])
    else:
      if y is None:
        self._model.fit(x, validation_data=val_data, batch_size=100, epochs=ep, verbose=1,  class_weight=class_weights,
                        callbacks=[callback])
      else:
        self._model.fit(x, y, batch_size=100, epochs=ep, validation_data=val_data, verbose=1, callbacks=[callback])
      ...
      try:
        if float(qualities[idx]) >= min_quality:
          if self._check_edges(start_xs[idx], end_xs[idx], start_ys[idx], end_ys[idx], dim, dp) is True:
            ...
            imageio.imwrite(outDir+[x for x in aux_fname.split(os.sep)][-1]+"_"+str(idx)+".jpeg", patch)
            patch = cv2.imread(outDir+[x for x in aux_fname.split(os.sep)][-1]+"_"+str(idx)+".jpeg", cv2.IMREAD_GRAYSCALE)
            if self._check_values(patch, px):
            ...
            print("Minutiae "+self._define_number(idx)+" of fingerprint "+aux_fname+" is too close to edges.")
      except:
        print("Minutiae "+self._define_number(idx)+" of fingerprint "+aux_fname+" not usable.")
